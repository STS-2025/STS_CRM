...
// 1️⃣ Total Leads
$stmt = $conn->prepare("SELECT COUNT(*) FROM leads WHERE status != 'converted' ");
$stmt->execute();
...
for ($i = 29; $i >= 0; $i--) {
    ...
    // Count deals
    $stmt = $conn->prepare("SELECT COUNT(*) as cnt FROM deals WHERE DATE(created_at)=?");
    if ($stmt === false) {
    ...
    // Count leads
    $stmt = $conn->prepare("SELECT COUNT(*) as cnt FROM leads WHERE DATE(created_at)=?");
    if ($stmt === false) {
    ...
    $stmt = $conn->prepare("SELECT SUM(amount) as total FROM deals WHERE DATE(created_at)=?");
    if ($stmt === false) {
...
$stmt = $conn->prepare("
    ...
    WHERE DATE(d.created_at) = ? AND d.stage = 'confirmed'
");
$stmt->bind_param("s", $today);
$stmt->execute();
$result = $stmt->get_result();
...
$stmt = $conn->prepare("
    ...
    FROM deals d
    JOIN users u ON d.owner_id = u.id
    WHERE DATE(d.updated_at) = ? AND d.stage = 'won'
");
$stmt->bind_param("s", $today);
$stmt->execute();
$result = $stmt->get_result();
...
$stmt = $conn->prepare("
    ...
    FROM deals d
    JOIN users u ON d.owner_id = u.id
    WHERE DATE(d.updated_at) = ? AND d.stage = 'lost'
");
$stmt->bind_param("s", $today);
$stmt->execute();
$result = $stmt->get_result();
...
<section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    ...
    <?php echo generate_metric_card('Total Deals', $new_deals, 'text-yellow-500', 'handshake'); ?>
